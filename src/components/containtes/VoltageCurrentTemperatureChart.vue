<template>
  <div>
    ...
    <div v-if="batteries.length == 0">
      ...
      <CardNoData message="Aucune batterie disponible" />
    </div>
...
export default {
    setup() {
        ...
        function filterDataByTimeRange(range) {
            ...
            switch (range) {
                case "Jour":
                    averagedData = calculateHourlyAverages(filteredTimeData, filteredVoltageData, filteredCurrentData, filteredTemperatureData);
                    break;
                case "Semaine":
                    averagedData = calculateWeeklyAverages(filteredTimeData, filteredVoltageData, filteredCurrentData, filteredTemperatureData);
                    break;
                case "Mois":
                    averagedData = calculateMonthlyAverages(filteredTimeData, filteredVoltageData, filteredCurrentData, filteredTemperatureData);
                    break;
                case "Année":
                    averagedData = calculateYearlyAverages(filteredTimeData, filteredVoltageData, filteredCurrentData, filteredTemperatureData);
                    break;
                default:
        ...
        function calculateHourlyAverages(timeData, voltageData, currentData, temperatureData) {
            let hourlyData = [];
            let currentHour = [];
            let currentVoltage = [];
            let currentCurrent = [];
            ...
            timeData.forEach((time, idx) => {
                ...
                const hoursDiff = Math.floor((currentDate - startOfDay) / (1000 * 60 * 60)); // diff en heures
                if (hoursDiff < 24) {
        ...
        function calculateWeeklyAverages(timeData, voltageData, currentData, temperatureData) {
            let weeklyData = [];
            let currentDay = [];
            let currentVoltage = [];
            let currentCurrent = [];
            ...
            timeData.forEach((time, idx) => {
                const currentDate = new Date(time);
                const daysDiff = Math.floor((currentDate - startOfWeek) / (1000 * 60 * 60 * 24)); // diff en jours
                if (daysDiff < 7) {
        ...
        function calculateMonthlyAverages(timeData, voltageData, currentData, temperatureData) {
            let monthlyData = [];
            let currentWeek = [];
            let currentVoltage = [];
            let currentCurrent = [];
            ...
            timeData.forEach((time, idx) => {
                const currentDate = new Date(time);
                const weeksDiff = Math.floor((currentDate - startOfMonth) / (1000 * 60 * 60 * 24 * 7)); // diff en semaines
                if (weeksDiff < 4) {
        ...
        function calculateYearlyAverages(timeData, voltageData, currentData, temperatureData) {
            let yearlyData = [];
            let currentMonth = [];
            let currentVoltage = [];
            let currentCurrent = [];
            ...
            timeData.forEach((time, idx) => {
                const currentDate = new Date(time);
                const monthsDiff = Math.floor((currentDate - startOfYear) / (1000 * 60 * 60 * 24 * 30)); // diff en mois
                if (monthsDiff < 12) {
