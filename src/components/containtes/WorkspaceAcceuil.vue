<template>
  <div class="container1">
      ...
      <div class="containerAcceuil">
        <template v-if="useParc.test">
          ...
          <template v-if="dataReceived.length > 0 && dataReceived.length <= 3">
            <BatteryItem
              v-for="(item, index) in dataReceived.length"
              :key="index"
              :title="titles[index]"
        ...
        <template v-else>
          <div
            ...
            :key="parcItem.id"
            class="cardItem bg-white border border-gray-200 shadow dark:bg-gray-800 dark:border-gray-700"
    ...
    <!-- 
    <template v-if="!useParc?.parcSuperviser || !parcSuperviserLocal">
      <div class="containerAcceuil">
        <template
          v-if="
            useParc?.parcsData &&
            (useParc?.parcSuperviser || parcSuperviserLocal)
          "
        >
          <div
            v-for="parcItem in useParc.parcsData"
          ...
          >
            ...
            <div class="p-5">
              ...
              <a
                @click="useParc.parcSuperviserFunc(parcItem)"
                class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none colorBg"
              >
                Suppervisé
                <svg
